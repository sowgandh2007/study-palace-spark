...
export type HingePosition = 'top' | 'bottom' | 'left' | 'right';
export type SplitMode = 'char' | 'word' | 'line';
export type TriggerMode = 'mount' | 'hover' | 'scroll' | 'loop';
...
const HINGE_CONFIG: Record<HingePosition, { origin: string; rotateX: number; rotateY: number }> = {
...
export const FoldText: React.FC<FoldTextProps> = ({
  ...
  useEffect(() => {
    ...
    const pieces = Array.from(root.querySelectorAll('.fold-text-piece')) as HTMLElement[];
